...
export function CommandCenter() {
  ...
  const handleCommand = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const nearbyEvents = await calendarService.fetchNearbyEvents(
        user.uid,
        parsed.start_time,
      );
      ...
      if (conflictReport.conflict) {
        ...
        if (conflictReport.action === "reschedule_existing") {
          ...
          conflictReport.action === "suggest_alternative" &&
          conflictReport.suggested_start_time
        ) {
          ...
          toast.success(
            `Alternative Slotted: "${parsed.title}" scheduled for ${format(altStart, "PPp")}`,
            { id },
          );
          setPrompt("");
          return;
