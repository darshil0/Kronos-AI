...
describe("Integration: Feedback Submission Flow", () => {
  ...
  it("completes full flow: user fills form → submits → success shown", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Subject/i), {
      target: { value: "Operational Review" },
    });
    fireEvent.change(screen.getByLabelText(/Message/i), {
      target: { value: "Schedule optimization is working" },
    });

    // 5. Submit
    fireEvent.click(
      screen.getByRole("button", { name: /Submit Transmission/i }),
    );
  ...
  it("handles login mid-session simulation (email pre-fill triggers on render)", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByDisplayValue("new-user@kronos.ai"),
      ).toBeInTheDocument();
    });
  });
});
